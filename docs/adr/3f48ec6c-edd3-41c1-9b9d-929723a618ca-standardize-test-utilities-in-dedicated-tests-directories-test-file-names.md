# Standardize Test Utilities in Dedicated __tests__ Directories: Test File Names
...
1. MUST: Test file names MUST match the source file name with the .test suffix (e.g., sanitizeAndValidateInput.ts → sanitizeAndValidateInput.test.ts)

## Policy Block

- MUST Test file names MUST match the source file name with the .test suffix (e.g., sanitizeAndValidateInput.ts → sanitizeAndValidateInput.test.ts)
